...
const Navbar = () => {
    ...
    return (
        <nav className="w-full px-[5vw] py-[2vh] bg-transparent relative z-10 flex items-center justify-between font-Inter">
            ...
            <div className="hidden md:flex items-center gap-[10%] text-[1.1vw] font-medium translate-x-4/12">
                <NavLink
                    to="/"
            ...
            {isMenuOpen && (
                <div className="md:hidden absolute top-0 left-0 w-full bg-white shadow-lg py-4 flex flex-col items-center gap-4">
                    <NavLink
                        ...
                        onClick={toggleMenu}
                    >
                        <button className="uppercase text-lg hover:cursor-pointer hover:text-yellow-800">Home</button>
                    </NavLink>
                    <NavLink
                        to="/shop"
                        className={({ isActive }) => isActive ? "text-yellow-600" : ""}
                        onClick={toggleMenu}
                    >
                        <button className="uppercase text-lg hover:cursor-pointer hover:text-yellow-800">Shop</button>
                    </NavLink>
                    <NavLink
                        to="/about"
                        className={({ isActive }) => isActive ? "text-yellow-600" : ""}
                        onClick={toggleMenu}
                    >
                        <button className="uppercase text-lg hover:cursor-pointer hover:text-yellow-800">About</button>
                    </NavLink>
                    <NavLink
                        to="/contact"
                        className={({ isActive }) => isActive ? "text-yellow-600" : ""}
                        onClick={toggleMenu}
                    >
                        <button className="uppercase text-lg hover:cursor-pointer hover:text-yellow-800">Contact</button>
                    </NavLink>
